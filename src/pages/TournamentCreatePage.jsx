...
import { prizeRowsToStructure } from "../utils/prizeStructure";
...
function TournamentCreatePage() {
  ...
  const [pokerTournament, setPokerTournament] = useState({
    max_entries: "",
    max_reentries: "",
    max_addons: "",
  ...
  const validateCommon = () => {
    ...
    if (!startTime || !registrationDeadline) {
      alert("Tournament schedule is required.");

      return false;
    }

    if (Object.keys(prizeStructure).length === 0) {
      alert("Prize structure is required.");

      return false;
    }

    if (!images.primary) {
      alert("Representative image is required.");
  ...
  const validatePoker = () => {
    if (!pokerTournament.max_entries) {
      alert("Max Entries is required.");

      return false;
    }

    if (!pokerTournament.starting_chips) {
      alert("Starting Chips is required.");

      return false;
    }

    if (!pokerTournament.reentry_chips) {
      alert("Reentry Chips is required.");

      return false;
    }

    if (!pokerTournament.addon_chips) {
      alert("Addon Chips is required.");

      return false;
    }

    const validBlind = blindStructure.levels.some(
      (level) =>
        level.big_blind !== "" &&
        level.small_blind !== "" &&
    ...
    if (!validBlind) {
      alert("At least one blind level is required.");
  ...
  const buildPokerData = () => {
    return {
      max_entries: Number(pokerTournament.max_entries),

      max_reentries:
        pokerTournament.max_reentries === ""
          ? 0
          : Number(pokerTournament.max_reentries),

      max_addons:
        pokerTournament.max_addons === ""
          ? 0
          : Number(pokerTournament.max_addons),

      starting_chips: Number(pokerTournament.starting_chips),

      early_chips:
        pokerTournament.early_chips === ""
          ? 0
          : Number(pokerTournament.early_chips),

      reentry_chips: Number(pokerTournament.reentry_chips),

      addon_chips: Number(pokerTournament.addon_chips),
      ...
      reentry_fee:
        pokerTournament.reentry_fee === ""
          ? 0
          : Number(pokerTournament.reentry_fee),

      addon_fee:
        pokerTournament.addon_fee === ""
          ? 0
          : Number(pokerTournament.addon_fee),
    };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("start_time", formatDateTimeToApi(startTime));

    formData.append(
      "registration_deadline",
      formatDateTimeToApi(registrationDeadline),
    );

    formData.append(
      "prize_structure",
      JSON.stringify(prizeRowsToStructure(prizeStructure)),
    );

    if (gameType === "POKER") {
      formData.append("poker_tournament", JSON.stringify(buildPokerData()));
    }

    if (images.primary) {
      formData.append("images", images.primary);
    }

    images.others.forEach((image) => {
    ...
    try {
      ...
      navigate(`/shop-tournaments/${response.data.data.id}`);
    } catch (error) {
      console.error(error);

      alert(
        JSON.stringify(error.response?.data || "Tournament creation failed."),
      );
    }
  };
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        ...
        {gameType === "POKER" && (
          <>
            <PokerTournamentForm
              value={pokerTournament}
              onChange={setPokerTournament}
            />

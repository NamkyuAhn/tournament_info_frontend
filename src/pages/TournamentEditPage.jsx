...
function TournamentEditPage() {
  ...
  const [pokerTournament, setPokerTournament] = useState(null);
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      try {
        ...
        setRegistrationDeadline(data.registration_deadline.slice(0, 16));

        let prizeData = data.prize_structure;

        if (typeof prizeData === "string") {
          prizeData = JSON.parse(prizeData);
        }

        setPrizeStructure(prizeStructureToRows(prizeData));
        ...
        if (data.poker_tournament) {
          ...
          setBlindStructure(data.poker_tournament.blind_structure);
        }

        setImages({
          primary: data.images?.find((image) => image.is_primary) || null,

          others: data.images?.filter((image) => !image.is_primary) || [],
        ...
        alert("Tournament loading failed.");
      }
    };
  ...
  const buildPokerData = () => {
    return {
      ...
      max_reentries: Number(pokerTournament.max_reentries || 0),

      max_addons: Number(pokerTournament.max_addons || 0),

      starting_chips: Number(pokerTournament.starting_chips),

      early_chips: Number(pokerTournament.early_chips || 0),
      ...
      reentry_fee: Number(pokerTournament.reentry_fee || 0),

      addon_fee: Number(pokerTournament.addon_fee || 0),
    };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (images.changed) {
      ...
      const existingImageIds = [
        ...(images.primary?.id ? [images.primary.id] : []),

        ...images.others.filter((image) => image?.id).map((image) => image.id),
      ];

...
function MyTournamentDetailPage() {
  ...
  useEffect(() => {
    const fetchTournament = async () => {
      try {
        const response = await api.get(`/tournaments/my-tournaments/${id}/`);
  ...
  const handleReentry = async () => {
    try {
      ...
      alert("Reentry request submitted.");

      window.location.reload();
    } catch (error) {
      console.error(error);

      alert(JSON.stringify(error.response?.data || "Reentry failed."));
    }
  };
  ...
  const handleAddon = async () => {
    try {
      ...
      alert("Addon request submitted.");

      window.location.reload();
    } catch (error) {
      console.error(error);

      alert(JSON.stringify(error.response?.data || "Addon failed."));
    }
  };
    ...
    <div>
      ...
      {tournament.poker_tournament && (
        <>
          ...
          <p>Starting Chips: {tournament.poker_tournament.starting_chips}</p>

          <p>Reentry Fee: {tournament.poker_tournament.reentry_fee}</p>

          <p>Addon Fee: {tournament.poker_tournament.addon_fee}</p>
      ...
      {tournament.entry && (
        <>
          <h2>My Entry Information</h2>

          <p>Approval: {tournament.entry.approval_status}</p>

          <p>Entry Status: {tournament.entry.status}</p>

          <p>Table: {tournament.entry.table_number}</p>

          <p>Seat: {tournament.entry.seat_number}</p>

          <h2>Buy-in History</h2>

          {tournament.entry.buyin_events.map((event) => (
            <p key={event.id}>
              {event.type} - {event.amount} ({formatDateTime(event.created_at)})
            </p>
          ))}
        </>
      )}

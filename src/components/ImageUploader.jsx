...
function ImageUploader({ value, onChange }) {
  ...
  useEffect(() => {
    ...
    setPrimaryImage(value.primary ?? null);

    setOtherImages(value.others ?? []);
  }, [value]);

  const isOldImage = (image) => {
    return image && typeof image === "object" && image.image;
  };
  ...
  const emitChange = (primary, others) => {
    onChange({
      primary,
  ...
  const handleOtherImages = (e) => {
    ...
    const updated = [...otherImages, ...files];

    setOtherImages(updated);

    emitChange(primaryImage, updated);
  };

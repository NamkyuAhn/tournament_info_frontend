...
function BlindStructureEditor({ value, onChange }) {
      ...
      setLevels(
        value.levels.map((item) => {
          ...
          return {
            ...
            ante: item.ante === 0 ? "" : String(item.ante),
  ...
  const updateLevel = (index, field, inputValue) => {
  ...
  const addLevel = () => {
    const nextLevelNumber =
      levels.filter((item) => item.type === "LEVEL").length + 1;

    const updatedLevels = [...levels, createLevel(nextLevelNumber)];
  ...
  const deleteLevel = (index) => {
    ...
    const updatedLevels = filtered.map((item) => {
      if (item.type === "BREAK") {
        return item;
      }
      ...
      levels: data
        ...
        .map((item) => {
          ...
          return {
            ...
            big_blind: Number(item.big_blind),

            small_blind: Number(item.small_blind),

            ante: item.ante === "" ? 0 : Number(item.ante),
    ...
    >
      ...
      <div
        style={{
          ...
          gridTemplateColumns: "60px 0.8fr 0.8fr 0.8fr 0.9fr 150px",
            ...
            >
              ...
              <div>
                <input
                  ...
                  onChange={(e) =>
                    updateLevel(index, "duration_minutes", e.target.value)
                  }
                />
        ...
        return (
          ...
          >
            ...
            <input
              ...
              placeholder="SB"
              onChange={(e) =>
                updateLevel(index, "small_blind", e.target.value)
              }
            />
            ...
            >
              ...
              <button type="button" onClick={() => addBreak(index)}>
                + Break
              </button>
            </div>
          </div>
